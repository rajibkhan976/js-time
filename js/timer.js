(function ($) {
  ...
  function prepareFlipItems(flipContainer) {
    ...
    var timestamp = 0;
    var itemCount = 0;
	function getSalatTime (responseData, flipContainer, flipElements, timestamp, itemCount) {
    ...
    flipContainer.children().each(function () {
      var element = $(this);
      var number = parseInt(sehriArr.pop());
	...
	async function setSalatTime () {
		...
		), flipContainer, flipElements, timestamp, itemCount);
	}
	setSalatTime();
    return {
